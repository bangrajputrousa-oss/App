...
import { ArrowLeft, Award, Copy, Check } from 'lucide-react';
...
export const LicenseScreen: React.FC<LicenseScreenProps> = ({
  ...
  const licenseDoc = appState.documents.find((d) => d.type === 'license') || {
    id: 'doc-2',
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(licenseDoc.number);
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  };

  return (
    <div id="screen-my-driving-license" className="flex-1 flex flex-col overflow-y-auto bg-[#131416] text-white">
        ...
        <div className="w-full bg-[#222428] rounded-2xl overflow-hidden shadow-lg border border-neutral-800/80 p-5 flex flex-col gap-5 text-sm">
          ...
          <div className="flex items-center justify-between">
            <div className="flex flex-col gap-1">
              <span className="text-neutral-400 text-xs">License Number</span>
          ...
          <div className="flex flex-col gap-1">
            ...
            <span className="text-white font-semibold text-sm">Private Vehicle</span>
          </div>

          <div className="flex flex-col gap-1">
            <span className="text-neutral-400 text-xs">Issuing Date</span>
            <span className="text-white font-semibold text-sm font-mono">{licenseDoc.issuing}</span>
          </div>

          <div className="flex flex-col gap-1">
            <span className="text-neutral-400 text-xs">Expiry Date</span>
            <span className="text-white font-semibold text-sm font-mono">{licenseDoc.expiry}</span>
          </div>
